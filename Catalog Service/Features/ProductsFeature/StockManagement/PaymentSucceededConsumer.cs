...
namespace Catalog_Service.Features.ProductsFeature.StockManagement
{
    public class PaymentSucceededConsumer : IConsumer<PaymentSucceededEvent>
    {
        ...
        public async Task Consume(ConsumeContext<PaymentSucceededEvent> context)
        {
            ...
            _logger.LogInformation("Processing stock deduction for Paid Order {OrderId}", msg.OrderId);
            ...
            foreach (var item in msg.Items)
            {
                ...
                {
                    _logger.LogWarning("Product {ProductId} from Paid Order {OrderId} not found during stock deduction", item.ProductId, msg.OrderId);
                }
            }

            await _unitOfWork.SaveChangesAsync(context.CancellationToken);
            _logger.LogInformation("Stock deduction completed for Paid Order {OrderId}", msg.OrderId);
        }
    }
}
